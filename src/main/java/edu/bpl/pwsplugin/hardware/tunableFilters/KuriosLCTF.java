...
public class KuriosLCTF extends DefaultTunableFilter {
   ...
   public void initialize() throws MMDeviceException {
      super.initialize();
      try {
         Globals.core().setDeviceDelayMs(this.devName,
               10);  // Testing has shown that this delay (taken into account by the acquisition manager) helps reduce noise.
      } catch (Exception e) {
      ...
      try {
         ...
         if (!prop.equals("Ready")) {
            errs.add(String.format(
                  "Kurios LCTF indicates that the device status is `%s` rather than `Ready`, please wait.",
                  prop));
         }
