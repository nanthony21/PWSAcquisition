...
public class Spectra3 implements TunableFilter {
    ...
    public void setWavelength(int wavelength) throws MMDeviceException {
        ...
        for (Map.Entry<String, Integer> entry : wvMap.entrySet()) {
            ...
            try {
                Globals.core().setProperty(this._settings.name, entry.getKey(), 0); // Turn off all wavelengths
            } catch (Exception e) {
        ...
        try {
            ...
            Globals.core().setProperty(this._settings.name, closestPropName, 1); // Turn on the closest wavelength
        } catch (Exception e) {
            throw new MMDeviceException(e);
        }
    }
    ...
    public void activate() throws MMDeviceException {
        try {
            for (Map.Entry<String, Integer> entry : wvMap.entrySet()) {
                ...
                Globals.core().setProperty(devName, String.format("%s_Intensity", entry.getKey()), 1000.0); // Turn intensity all the way up.
            }
