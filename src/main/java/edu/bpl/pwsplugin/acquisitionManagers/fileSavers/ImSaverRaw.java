...
public class ImSaverRaw extends SaverThread {
   ...
   public void run() {
      try {
         ...
         im = (Image) getQueue()
               .poll(5, TimeUnit.SECONDS); //Lets make an array with the queued images.
         if (im == null) {
         ...
         for (int i = 1; i < expectedFrames_; i++) {
            im = (Image) getQueue()
                  .poll(1, TimeUnit.SECONDS); //Lets make an array with the queued images.
            if (im == null) {
               ReportingUtils.showError("ImSaver timed out while waiting for image");
               return;
            }
         ...
         boolean success = saver
               .saveAsTiffStack(Paths.get(savePath_).resolve(filePrefix_ + ".tif").toString());
         if (!success) {
         ...
         ReportingUtils.logError(
               "Error: PWSPlugin, while producing " + filePrefix_ + " image: " + ie.toString());
      } catch (JSONException | IOException ex) {
         ReportingUtils.showError(ex);
         ReportingUtils.logError(
               "Error: PWSPlugin, while producing " + filePrefix_ + " image: " + ex.toString());
      }
   }
