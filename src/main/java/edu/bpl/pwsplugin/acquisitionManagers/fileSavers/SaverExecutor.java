...
/**
 * Each time a new image is requested to be saved a new thread will be created to handle it. TODO:
 * The usage of static variables was probably not smart. This should be split into two classes. One
 * is the executor that handles executing tasks on a single thread. The other class represents the
...
public abstract class SaverExecutor implements ImageSaver, Callable<Void> {
   ...
   static {
      ...
      timer.scheduleAtFixedRate(task, 1000,
            1000);// 1000, actlist); //This timer checks the results of the saver threads on a regular interval. It will call its target from the AWT eventqueue thread.
   }
   ...
   public void beginSavingThread() throws InterruptedException, ExecutionException {
      ...
      if (initialized) {
         throw new RuntimeException(
               "This ImageSaver has already been run once. You must create a new one.");
      }
      initialized = true;
      ...
      synchronized (threadFutures) { // Make sure this list is only modified by one thread at a time.
         threadFutures
               .add(future); //We used to allow multiple saving threads at once, this led to terrible write speed. Better to feed all tasks to a single thread.
      }
   }
   ...
   private static void processRunningFutures() throws InterruptedException, ExecutionException {
      synchronized (threadFutures) { // Make sure no other thread modifies the list while we're working with it.
         ...
         while (iter
               .hasNext()) {  // Since we are modifying the list as we loop through it we need to use iterator rather than a normal for-loop.
         ...
         if (threadFutures.size() > 2) {
            Globals.logger().logMsg(
                  String.format("Warning: SaverExecutor has %d jobs queued", threadFutures.size()));
         }
      }
   }
