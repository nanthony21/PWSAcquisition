...
class SettingsPanel extends JPanel implements TreeSelectionListener, FocusListener, MouseListener {
   ...
   private void saveSettingsOfLastNode() {
      if (this.lastSelectedNode != null) {
         ...
         try {
            settings = (JsonableParam) panelTypeMapping.get(this.lastSelectedNode.getType())
                  .build();
   ...
   public void focusGained(
         FocusEvent evt) { // Clicking from one JTree to another one doesn't fire a TreeSelectionEvent. Force one to happen so the panel always shows the right settings
      saveSettingsOfLastNode();
   ...
   public void focusLost(
         FocusEvent evt) {  //When the user clicks on any other component than one of the Trees make sure to save settings.
      saveSettingsOfLastNode();
   }

   @Override
   public void mouseExited(
         MouseEvent e) { //The focus listeners still sometimes miss an event where we need to save settings.
      Rectangle r = e.getComponent().getBounds();
      Point p = e.getPoint(); // This point is relative to the component origin.
      if (p.x < 0 || p.x >= r.width || p.y < 0 || p.y
            >= r.height) { // The mouse exited event fires even if the mouse is actually on a subcomponent of this one. Only fire if the mouse is outside the component.
         saveSettingsOfLastNode();
      }
   }
