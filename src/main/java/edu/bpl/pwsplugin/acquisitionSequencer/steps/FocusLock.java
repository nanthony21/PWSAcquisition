...
public class FocusLock extends ContainerStep<SequencerSettings.FocusLockSettings> {
   ...
   protected SequencerFunction getCallback() {
      return (status) -> {
         Step[] path = status.coords()
               .getTreePath(); //Indicates our current location in the tree of steps.
         if (path[path.length - 1].getType().equals(SequencerConsts.Type.ACQ
               .name())) { //If the current  step is an acquisition then check for refocus.
            TranslationStage1d zStage = Globals.getHardwareConfiguration().getActiveConfiguration()
                  .zStage();
   ...
   public SequencerFunction getStepFunction(List<SequencerFunction> callbacks) {
      ...
      return (status) -> {
         ...
         TranslationStage1d zstage = Globals.getHardwareConfiguration().getActiveConfiguration()
               .zStage();
         try {
            double startingZ = zstage
                  .getPosUm(); //After finding focus lock we will move back to this z position.
            zstage.runFullFocus();
            Thread.sleep(
   ...
   public List<String> validate() {
      ...
      while (en.hasMoreElements()) {
         ...
         } else if (step.getType().equals(SequencerConsts.Type.AF
