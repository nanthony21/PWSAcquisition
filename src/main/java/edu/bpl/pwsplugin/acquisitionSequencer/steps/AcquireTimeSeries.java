 ...
 * @author nick
 */
public class AcquireTimeSeries extends
      IteratingContainerStep<SequencerSettings.AcquireTimeSeriesSettings> {
      ...
      return new SequencerFunction() {
         ...
         public AcquisitionStatus applyThrows(AcquisitionStatus status) throws Exception {
            ...
            for (currentIteration = 0; currentIteration < settings.numFrames; currentIteration++) {
               ...
               lastAcqTime = System
                     .currentTimeMillis(); //Save the current time so we can figure out when to start the next acquisition.
               status = stepFunction.apply(status);
               status.newStatusMessage(
                     String.format("Finished time step %d of %d", currentIteration + 1,
                           settings.numFrames));
            }
            return status;
         }
