...
class FactoryRegistry {
    ...
    public void registerFactory(String typeName, StepFactory factoryInstance) throws IllegalArgumentException {
        if (REGISTRY.containsKey(typeName)) {
            throw new IllegalArgumentException(String.format("A StepFactory of type %s has already been registered.", typeName));
        }
        REGISTRY.put(typeName, factoryInstance);
    }
    
    public StepFactory getFactory(String typeName) { // Will return null if a factory hasn't been registered for this typeName
