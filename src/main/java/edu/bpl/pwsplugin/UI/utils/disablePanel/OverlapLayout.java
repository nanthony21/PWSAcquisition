 ...
 * in an overlapping fashion. A component can be painted "above" or "below" the previous component
 * in the container.
 * <p>
 * Like the GridLayout, each component is sized to the largest width and height of any component in
 * the container. The amount of overlap is controlled by specifying the overlap postion of each
 * component. You can simulate, left-to-right, right-to-left, top-to-bottom, bottom-to-top and
 * diagonal layouts. As well you can "stack" components completely on top of one another. In this
 * case the components are sized to the space available in the container.
 * <p>
 * A main usage for this layout might be in the creation of "card games". A few features have been
 * added that might be handy in these cases:
 * <p>
 * a) a "popup" feature - when a component is selected in can "popup" from its regular location so
 * it visibly stands out. To accomplish this some extra space must be reserved in the container for
 * the popup. This is done by using the setPopupInsets method which allow you to control the popup
 * direction. In addition you can add/remove a simple constraint to the component. POP_UP will popup
 * the component. POP_DOWN or null will paint the component in its regular location. b) when a
 * component is made "invisible" you can reserve its location in the container so all the other
 * components don't shift.
 * <p>
 * Note: this layout is achieved by changing the ZOrder of components in the container. It will not
 * work for all components as some compnents will always paint themselves on the top of others. This
 * seems to happen with components like JButton as rollover effects are painted when a mouse moves
 * over the components.
 */
public class OverlapLayout implements LayoutManager2, java.io.Serializable {
    ...
    * stacked above the previous component and sized to fill the space of the parent container.
    */
   ...
   /**
    * When components are overlapped above the ZOrder of each component is changed resulting in the
    * components position in the container being changed. For example when you add a component to
    * the end of the container it will be moved to the beginning. If you then try to access the
    * component using Component.componentAt(), you will get the first component, not the last.
    * <p>
    ...
    *
    * @param includeInvisible when true, space is reserved otherwise the component is not included
    *                         in the layout sizing
    */
   ...
   /**
    * Define extra space to be reserved by the container. This will allow components to be "popped
    * up" if required. Generally space would only be reserved on one side of the container.
    *
    * @param popupInsets Insets defining extra space for a particular side of the container.
    */
   ...
   public void layoutContainer(Container parent) {
      synchronized (parent.getTreeLock()) {
         ...
         if (overlapPosition.x < 0) {
            location.x =
                  parent.getWidth() - maximumSize.width - parentInsets.right - popupInsets.right;
         } else {
         ...
         if (overlapPosition.y < 0) {
            location.y = parent.getHeight() - maximumSize.height - parentInsets.bottom
                  - popupInsets.bottom;
         } else {
