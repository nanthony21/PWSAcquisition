         ...
         if (type == DefaultMMFunctions.MMAcquisitionSettings) {
            return new Function<Void, File>() {
               ...
               public File apply(Void v) {
                  FileDialogs.FileType ACQ_SETTINGS_FILE = new FileDialogs.FileType(
                        "ACQ_SETTINGS_FILE", "Acquisition settings",
                        System.getProperty("user.home") + "/AcqSettings.txt",
                        true, "txt");
                  JFrame topFrame = (JFrame) SwingUtilities.getWindowAncestor(This);
                  return FileDialogs
                        .openFile(topFrame, "Load acquisition settings", ACQ_SETTINGS_FILE);
               }
            };
            ...
            return new Function<Void, File>() {
               ...
               public File apply(Void v) {
                  JFrame topFrame = (JFrame) SwingUtilities.getWindowAncestor(This);
                  return FileDialogs
                        .openDir(topFrame, "Dataset save location", FileDialogs.MM_DATA_SET);
               }
            };
         } else {
            return null;
