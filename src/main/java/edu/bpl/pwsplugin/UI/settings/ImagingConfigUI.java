...
public class ImagingConfigUI extends SingleBuilderJPanel<ImagingConfigurationSettings> {
   ...
   public ImagingConfigUI() {
      ...
      p.add(filtSettings);
      DisabledPanel filtSettingsPanel = new DisabledPanel(
            p);//Used to block the filter settings when they are not applicable.
      ...
      this.typeCombo.addActionListener((evt) -> {
         if (this.typeCombo.getSelectedItem() == ImagingConfiguration.Types.StandardCamera) {
            filtSettingsPanel
                  .setEnabled(false); // filter settings do not apply to a standard camera.
         } else {
      ...
      this.configGroup.addItemListener((evt) -> {
         String[] confs = Globals.core()
               .getAvailableConfigs((String) this.configGroup.getSelectedItem()).toArray();
      ...
      this.configGroup.getItemListeners()[0]
