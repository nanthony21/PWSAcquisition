...
public class PauseButton extends JButton{
    ...
    public PauseButton(boolean allowMDAPause) {
        ...
        this.addActionListener((e)->{ //The following is what gets executed when the button is pressed.
            if (this.getText().equals(PAUSE)) { //Arm the button for pausing at the next opportunity
                this.setText(WAIT);
                if (Globals.mm().acquisitions().isAcquisitionRunning() && allowMDAPause) {//Pause the MDA if we are currently running an MDA acquision
                    Globals.mm().acquisitions().setPause(true);
                    while (!Globals.mm().acquisitions().isPaused()) {} //Wait for it to actually pause
                    this.setText(RESUME);
                }
            } else if (this.getText().equals(WAIT)) { //If the button is armed then cancel.
                this.setText(PAUSE);
            } else if (this.getText().equals(RESUME)) { //If the pause button is currently pausing the program then break the pause and go back to the default mode.
                if (Globals.mm().acquisitions().isAcquisitionRunning() && allowMDAPause) { //Resume the MDA
