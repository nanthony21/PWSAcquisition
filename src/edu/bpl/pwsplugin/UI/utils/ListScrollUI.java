...
public class ListScrollUI<T extends List<S>, S extends JsonableParam> extends ListBuilderJPanel<T> implements MouseListener {
    ...
    public ListScrollUI(Class<T> clazz, S... defaultStepTypes) {
        ...
        JScrollPane scroll = new JScrollPane(this.scrollContents, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
    ...
    public void addStepAction(int position) throws BuilderPanelException {
        if (this.defaultStepTypes.length > 1) { //We have multiple subtypes of S that could be added.
            ...
            menu.show(this.scrollContents,(int) pp.getX()-10,(int) pp.getY()-10); //show the menu 0,0 from the add button
        } else {
